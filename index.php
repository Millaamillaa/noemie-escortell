<?php require_once'inc/header.php'; ?>
    <!-- le logo -->
    <a class="logo" href="#"> </a>

    <div class="imghome"></div>
    
    <!-- bloc 1-->

    <!-- <section id="bloc1">
        <div class="column">
            <div class="col"> 
                <img src="img/p1-logo.png" alt="logo-intro" class="noemie">
            </div> 
        </div>
    </section> -->
    

    <!-- bloc 2-->
    <!-- <section id="bloc2">
        <div class="column">
            <div class="col textbloc2"> 
                    <p class="spaceb"> &nbsp; ACCOMPAGNEMENT AU MONTAGE
                    <br> DE PROJETS EUROPÉENS ET DE COOPÉRATION </p>
                    <img src="img/p1-barre-separation.png" alt="separation">
                
                <div class="textsuite">  
                    <p class="spaceb"> 
                        <img src="img/triangle-g.png" alt="triangle"> &nbsp; Coopérer / Innover / Se Développer &nbsp; <img src="img/triangle-d.png" alt="triangle">
                    </p>
                </div>  
            </div>
        </div>
    </section> -->

    <!-- bloc 3-->
    <!-- <section id="bloc3">
        <div class="container">
            <div class="row imgbloc3">
                
                <div class="col-xs-12 col-sm-6 col-md-6 col-lg-12">
                    <img src="img/p2-barreseparationbold.png" alt="separation">
                </div>
                <div class="textbloc3 col-xs-12 col-sm-6 col-md-6 col-lg-12 col-lg-offset-3">
                    <br>
                    <p> POUR S'OUVRIR SUR LE MONDE
                    <br> POUR ALLER PLUS LOIN, ENSEMBLE </p>
                    <br>
                </div>
                <div class="col-xs-12 col-sm-6 col-md-6 col-lg-12">
                     <img src="img/p2-barreseparationregular.png" alt="separation">
                </div> 
            </div>
        </div>
    </section>
    <div id="separateurorange"></div> -->
    
     <!-- bloc 4-->
    <!-- <section id="bloc4">
        <br>
        <p class="titlebloc">DERNIÈRES RÉALISATIONS </p>
    </section> -->
        <!-- les images en colones -->
        <div class="endfile container">
            <div class="row">
                <!-- image 1 --> 
                <div class="actua col-xs-9 col-sm-9 col-md-6 col-lg-3">
                    <div class="textbloc4">
                            ELABORATION DE STRATÉGIE 
                            EUROPÉENNE APPUI TECHNIQUE À LA 
                            RÉDACTION ET AU MONTAGE 
                    </div>
                    <div class="bloc_orange"> </div>
                    <img class="lineimage" src="img/p3-vignette-1.jpg" name="image" onmouseover="image.src='img/p3-vignette-2.jpg'" onmouseout="image.src='img/p3-vignette-1.jpg'" alt="vignette" width="100%"/>
                </div>
            <!-- image 2 -->
                <div class="actua col-xs-9 col-sm-9 col-md-6 col-lg-3">
                    <img class="lineimage" src="img/p3-vignette-2.jpg" name="secondimage" onmouseover="secondimage.src='img/p3-vignette-3.jpg'" onmouseout="secondimage.src='img/p3-vignette-2.jpg'" alt="vignette" width="100%"/>
                    <div class="bloc_orange"> </div>
                    <div class="textbloc4">
                        <p> APPUI TECHNIQUE SUR LA 
                            COMMUNICATION ET LA GESTION ADMINISTRATIVE
                        </p>
                    </div>
                </div>
                <!-- image 3 -->
                <div class="actua col-xs-9 col-sm-9 col-md-6 col-lg-3">
                    <div class="textbloc4">
                        <p> APPUI TECHNIQUE À L'ÉCRITURE, AU
                            DÉPÔT ET SUIVI ADMINISTRATIF
                        </p>
                    </div>
                    <div class="bloc_orange"> </div>
                    <img class="lineimage" src="img/p3-vignette-3.jpg" name="thridimage" onmouseover="thridimage.src='img/p3-vignette-1.jpg'" onmouseout="thridimage.src='img/p3-vignette-3.jpg'" alt="vignette" width="100%"/>
                </div> <!-- fin de col -->
            </div> <!-- fin de row -->
        </div> <!-- fin de container -->
    
  
<?php  require_once 'inc/footer.php';?>

